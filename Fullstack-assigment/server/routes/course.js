...
// const { validateToken } = require('../middlewares/auth'); // Remove validateToken import

// POST /courses endpoint to create a new course
router.post("/", async (req, res) => { // Removed validateToken middleware
    try {
        let data = req.body;
...
router.put("/:id", async (req, res) => { // Removed validateToken middleware
    try {
        ...
        let [num] = await Course.update(data, { where: { id: id } });
...
router.delete("/:id", async (req, res) => { // Removed validateToken middleware
    try {
        let id = req.params.id;
        const course = await Course.findByPk(id);
...
router.delete("/", async (req, res) => { // Removed validateToken middleware
    try {
