...
function Reports() {
    ...
    const getReports = () => {
        http.get('/report')
            .then((res) => {
                ...
                toast.dismiss();
                toast.info('Report list updated successfully');
            })
            .catch((err) => {
    ...
    useEffect(() => {
        // Check for state passed from Add/Edit Report pages
        if (location.state) {
                        ...
                        <TableBody>
                            {filteredReports.slice(0, visibleReports).map((report) => (
                                <TableRow key={report.id}>
                                    <TableCell>{dayjs(report.createdAt).format(global.datetimeFormat)}</TableCell>
