...
import {
    Box, Typography, Grid, Card, CardContent, Input, IconButton, Select, MenuItem, FormControl, InputLabel, Divider
} from '@mui/material';
import {
...
function Courses() {
    ...
    const searchCourses = async () => {
        try {
            const response = await axios.get(`http://localhost:3001/courses?search=${search}`);
            console.log('Search response data:', response.data); // Log the search response data
            setCourses(response.data);
            processCategoriesAndSkills(response.data);
            setFilteredCourses(response.data); // Update filteredCourses after search
        } catch (error) {
    ...
    // Function to render course status icon and color
    ...
    return (
        <Box>
            ...
            {/* Conditional rendering based on filteredCourses length */}
